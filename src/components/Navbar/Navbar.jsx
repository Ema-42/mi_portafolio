    ...
    <div className="flex justify-between items-center w-full h-14 px-4 text-white bg-black bg-opacity-60 backdrop-blur-lg fixed z-50 filter">
      ...
      <ul className="hidden md:flex">
        {links.map(({ id, link }) => (
          <li
            key={id}
            className="px-4 cursor-pointer capitalize font-medium text-gray-200 hover:text-white "
          >
      ...
      {nav && (
        <ul className="flex flex-col justify-center items-center absolute top-0 left-0 w-full h-screen bg-gradient-to-b from-black to-gray-800 text-gray-500 ">
          {links.map(({ id, link }) => (
            <li
              key={id}
              className="px-4 cursor-pointer capitalize py-6 text-4xl hover:scale-105 duration-200 hover:text-white"
            >
              <Link
                onClick={() => setNav(!nav)}
                to={link}
